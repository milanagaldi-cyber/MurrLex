...
- Android visible app version: `MurrLex v0.89`
- Latest verified Android build command: `gradlew.bat :app:packageDebug --no-daemon` completed for v0.89
- Latest verified lint command: not rerun for v0.89
...
- v0.89 moves online AI traffic behind authenticated MurrLex server endpoints. The mobile app stores only its server session and selected models; OpenAI and ElevenLabs provider keys stay in server environment variables. Online text, STT, TTS, and OCR use `/api/ai/*`; offline behavior remains Android/ML Kit/device TTS and cached audio.
